...
// ============================================================================
// 1. NAIVE REPOSITORY (VARCHAR(255) Key, JSON Bytea Value)
// ============================================================================

type NaiveRepository struct {
	pool *pgxpool.Pool
}

// NewNaiveRepository creates a new naive PostgreSQL repository.
func NewNaiveRepository(pool *pgxpool.Pool) ports.NaiveUserRepository {
	return &NaiveRepository{
		pool: pool,
	}
}

// Get retrieves a UserData by its string key into a destination struct using a prepared statement and cached global time.
func (r *NaiveRepository) Get(ctx context.Context, key string, dest *domain.UserData) (bool, error) {
	var value []byte

	dbCtx, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()

	// Executing connection-level prepared statement "get_user_naive"
	...
	if err != nil {
		...
		return false, fmt.Errorf("naive get db error: %w", err)
	}

	// Naive deserialization directly into the recycled object
	if err := json.Unmarshal(value, dest); err != nil {
		return false, fmt.Errorf("naive json decode error: %w", err)
	}

	return true, nil
}

// Set stores a UserData serialized to JSON with a string key using a prepared statement and cached global time.
func (r *NaiveRepository) Set(ctx context.Context, key string, user *domain.UserData, ttl time.Duration) error {
	expiresAt := domain.CurrentTimeUTC.Load().(time.Time).Add(ttl)

	// Naive serialization using standard reflection-based json.Marshal
	value, err := json.Marshal(user)
	if err != nil {
		return fmt.Errorf("naive json encode error: %w", err)
	}

	dbCtx, cancel := context.WithTimeout(ctx, 2*time.Second)
	defer cancel()

	// Executing connection-level prepared statement "set_user_naive"
	...
	if err != nil {
		return fmt.Errorf("naive set db error: %w", err)
	}

	return nil
}

type naiveCopySource struct {
...
func (s *naiveCopySource) Next() bool {
	if s.index >= len(s.users) {
		return false
	}
	u := &s.users[s.index]

	keyStr := domain.UUIDToString(u.ID)
...
func (s *naiveCopySource) Values() ([]any, error) {
	return s.rowValues, nil
}

func (s *naiveCopySource) Err() error {
	return s.err
}

// Seed populates the database using high-performance pgx CopyFrom bulk copy.
func (r *NaiveRepository) Seed(ctx context.Context, users []domain.UserData, ttl time.Duration) error {
	expiresAt := time.Now().Add(ttl).UTC()

	source := &naiveCopySource{
		users:     users,
		expiresAt: expiresAt,
	...
	_, err := r.pool.CopyFrom(
		ctx,
		pgx.Identifier{"cache_naive"},
		[]string{"key", "value", "expires_at"},
		source,
	)
	if err != nil {
		return fmt.Errorf("failed to bulk copy naive seed: %w", err)
	}
	return nil
}
