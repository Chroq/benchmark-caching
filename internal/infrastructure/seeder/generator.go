...
// GenerateKeys constructs the dataset of distinct keys.
...
func GenerateKeys(count int) [][16]byte {
	...
	if err == nil {
		slog.Info("Loading pre-generated keys...", "path", keysPath)
		...
		for scanner.Scan() && len(globalKeys) < count {
			...
			if len(line) == 26 {
				...
				if tsidVal, err := strconv.ParseInt(line, 10, 64); err == nil {
					var key [16]byte
